...
ap.add_argument("--width", type=int, default=128, help="image resize width/height")
ap.add_argument("--landmark-path", default="../data/shape_predictor_68_face_landmarks.dat", help="landmark predictor")
args = ap.parse_args()
...
def align(img_path, width=128):
    img = Image.open(img_path)
    img = img.convert('RGB')  # if image is RGBA or Grayscale etc
    img = np.array(img)
    x, face_found = align_face(img)
    x = imutils.resize(x, width=width)
    return x

img = align(args.path, args.width)
Image.fromarray(img).save(args.path + '.jpg')
